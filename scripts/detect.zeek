...
export {
	...
	const dangerous_patterns = /xp_cmdshell/ | /sp_oacreate/ | /sp_oamethod/ |
	    /sp_oagetproperty/ | /xp_regread/ | /xp_regwrite/ |
	    /xp_regdeletekey/ | /xp_regenumvalues/ | /xp_instance_reg/ |
	    /xp_dirtree/ | /xp_fileexist/ | /xp_subdirs/ | /xp_getfiledetails/ |
	    /xp_servicecontrol/ | /sp_addextendedproc/ |
	    /sp_execute_external_script/ | /sp_replwritetovarbin/ | /sp_configure[^;]*(xp_cmdshell|ole automation|clr enabled|show advanced|ad hoc distributed|external scripts)/
	    | /openrowset/ | /opendatasource/ | /bulk insert/ |
	    /sp_addlinkedserver/ | /sp_addlinkedsrvlogin/ | /create assembly/ |
	    /sp_procoption/ | /trustworthy on/ | /sp_setapprole/ | /sp_password/ |
	    /alter login/ | /create login/ | /sp_addlogin/ |
	    /sp_addsrvrolemember/ | /alter server role[^;]*add member/ |
	    /sp_addrolemember/ | /sp_grantlogin/ | /xp_loginconfig/ |
	    /sys\.sql_logins/ | /password_hash/ | /sys\.syslogins/ |
	    /master\.\.sysxlogins/ | /waitfor delay/ | /sp_add_job/ |
	    /sp_add_jobstep/ | /sp_start_job/ |
...
event zeek_init() &priority=5
	{
	local r1 = SumStats::Reducer($stream="tds.login.failure", $apply=set(
	    SumStats::SUM, SumStats::UNIQUE), $unique_max=double_to_count(
	    scan_threshold + 2));
	SumStats::create([$name="tds-login-bruteforce", $epoch=bruteforce_interval,
	    $reducers=set(r1), $threshold_val (key: SumStats::Key,
	    result: SumStats::Result) =
		{
		return result["tds.login.failure"]$sum;
		}, $threshold=bruteforce_threshold, $threshold_crossed (key: SumStats::Key,
		    result: SumStats::Result) =
		{
		local r = result["tds.login.failure"];
		NOTICE([$note=Login_Bruteforce, $src=key$host, $msg=fmt(
		    "%s failed %d SQL Server logins in %s against %d server(s)",
		    key$host, double_to_count(r$sum), bruteforce_interval,
		    r$unique), $identifier=cat(key$host)]);
		}]);

	local r2 = SumStats::Reducer($stream="tds.scan", $apply=set(SumStats::UNIQUE),
	    $unique_max=double_to_count(scan_threshold + 2));
	SumStats::create([$name="tds-scan", $epoch=scan_interval, $reducers=set(r2),
	    $threshold_val (key: SumStats::Key, result: SumStats::Result) =
		{
		return result["tds.scan"]$unique + 0.0;
		}, $threshold=scan_threshold, $threshold_crossed (key: SumStats::Key,
		    result: SumStats::Result) =
		{
		NOTICE([$note=Scan, $src=key$host, $msg=fmt("%s probed %d SQL Servers with PRELOGIN without logging in, in %s",
...
event TDS::log_tds_login(rec: LoginInfo)
	...
	if ( rec?$success && ! rec$success )
		SumStats::observe("tds.login.failure", [$host=rec$id$orig_h], [$str=cat(
		    rec$id$resp_h)]);

	if ( detect_cleartext_password && rec$password_in_clear )
		NOTICE([$note=Cleartext_Password, $id=rec$id, $uid=rec$uid, $msg=fmt(
		    "%s sent a SQL Server password outside TLS to %s as %s",
...
event TDS::message(c: connection, is_orig: bool, msg_type: count, len: count,
	...
	if ( ! is_orig
	    && msg_type == 4
	    && ( c$tds$rows >= large_result_rows || len >= large_result_bytes ) )
		NOTICE([$note=Large_Result, $conn=c, $msg=fmt(
		    "%s received a %d-row, %d-byte result from %s", c$id$orig_h,
		    c$tds$rows, len, c$id$resp_h), $n=c$tds$rows,
		    $identifier=cat(c$id$orig_h, c$id$resp_h)]);

	if ( detect_bulk_load && is_orig && msg_type == 7 )
		NOTICE([$note=Bulk_Load, $conn=c, $msg=fmt("%s bulk-loaded %d bytes into %s",
		    c$id$orig_h, len, c$id$resp_h), $identifier=cat(c$id$orig_h,
		    c$id$resp_h)]);
	}

# -- New applications ----------------------------------------------------------------------

global known_applications: set[addr, string]
    &create_expire=new_application_memory;
...
event TDS::log_tds_login(rec: LoginInfo) &priority=-3
	...
	NOTICE([$note=New_Application, $id=rec$id, $uid=rec$uid, $msg=fmt(
	    "first login to %s from application %s (user %s, host %s)",
	    rec$id$resp_h, app, rec?$username ? rec$username : "?",
	    rec?$hostname ? rec$hostname : "?"), $sub=app, $identifier=cat(
	    rec$id$resp_h, app)]);
	}
